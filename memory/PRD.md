...
## Iteration 40 (Group Chat Reactions)
- **Group Chat Reactions** — members can long-press any group chat bubble to open a reaction picker with 7 curated emojis: 👍 ❤️ 😂 😮 🔥 🎉 ⛳. Tapping an emoji toggles the caller's reaction; existing reactions render as inline chips beneath the bubble with a live count. Tapping a chip toggles the caller's own reaction on that emoji. Reactions are optimistic on the client with rollback on failure.
- **Backend**: `POST /api/groups/{group_id}/chat/{message_id}/react` (body `{emoji}`) toggles the caller's reaction. Emoji is validated against `ALLOWED_REACTIONS`. Non-members get 403, unknown message 404, invalid emoji 400. `GET /api/groups/{id}/chat` now returns a `reactions: {emoji: [user_ids]}` dict on every message; `POST /api/groups/{id}/chat` seeds `reactions: {}` on creation.
- **Frontend**: `ChatThread` now accepts an optional `reactToMessage` prop; when provided (group chat), long-press opens the emoji picker and inline chips render beneath each bubble. DM threads intentionally opt out of reactions for now (they're 1:1 and less noisy without them).
- **Tests**: `/app/backend/tests/test_iter40_group_chat_reactions.py` (15 tests, all green).
